##  call getAutoDailyReport4Inidcator(10011,'w01001','0,avgdata,mindata,maxdata,diffdata,amplidata',2,'2018-09-13');
drop procedure if exists getAutoDailyReport4Inidcator;
create procedure getAutoDailyReport4Inidcator(
	in v_stationid	int,
	in v_indicatorcode	varchar(50),
	in v_stattypes varchar(200),
	in v_collectType  int,
	in v_reportdate varchar(20),
	...
	out o_collectTime varchar(20),
	out o_avgdata double,
	out o_maxdata double,
	out o_maxtime varchar(20),
	out o_mindata double,
	out 
)
begin
	## 变量声明
	...
	DECLARE CUR1 CURSOR FOR SELECT TABLENAME,begintime,endtime 
	   FROM aiot_meta_table  
	   WHERE wpid = v_stationid 
	   AND TYPE = 1 AND BEGINTIME <= v_reportdate and endtime > v_reportdate;
	##查询站点 设备的报警码信息
	...
	SELECT mindata,maxdata,deviceid
    ...
    FETCH cur2 INTO i_alarmData,i_beginTime,i_endTime,i_deviceid;
    	if not no_more_tables then
    		...
    		set alarmSql = concat(alarmSql,' and date_format(collect_time,''%Y-%m-%d'') =date_format(''',v_reportdate,''',''%Y-%m-%d'')');
    ...
    OPEN cur3;
	REPEAT
    FETCH cur3 INTO i_mindata,i_maxdata,i_deviceid;
    	if not no_more_tables then
    		set rangeSql = concat(rangeSql,' when data<',i_mindata,' and deviceid = ',i_deviceid,' then ',i_mindata);
    		set rangeSql = concat(rangeSql,' when data>',i_maxdata,' and deviceid = ',i_deviceid,' then ',i_maxdata);
	    end if;
    UNTIL  no_more_tables = 1  END REPEAT;
    close cur3;
    ...
    FETCH cur1 INTO stablename,sbegintime,sendtime;
    	if not no_more_tables then
    		...
    		set v_insql = concat(v_insql,' and isactive = 1');
