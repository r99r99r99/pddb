## call getAutoMonthReport('10011','2018-09')
drop procedure if exists getAutoMonthReport;
create procedure getAutoMonthReport(
...
begin
	...
	if length(v_reportDate) <=7 then
		set v_reportDate = concat(v_reportDate,'-01');
	end if;
	...
	##开始编写日报内容
	select date_format(v_reportDate,'%Y年%m月') into i_reportDate;
	
	##开始编写标题
	insert into tmp_result(reportTitle)
	values(concat(i_stationname,i_reportDate,'月报'));
	...
	and date_format(begintime,'%Y-%m') <= date_format(v_reportdate,'%Y-%m')
	and date_format(endtime,'%Y-%m') > date_format(v_reportdate,'%Y-%m')
    ...
    set v_insql = concat(v_insql,' and date_format(collect_time,''%Y-%m'') = date_format(''',v_reportdate,''',''%Y-%m'')');
